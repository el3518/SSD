...
class MDAnetsf(nn.Module):
    ...
    def __init__(self, num_classes=31):
        ...
        self.cls_fc_son5 = nn.Linear(256, num_classes)
    ...
    def forward(self, data_src, label_src = 0, mark = 1):
        ...
        if self.training == True:
            ...
            if mark == 1:              
                ...
                cls_loss += 0.5*F.nll_loss(F.log_softmax(pred_src, dim=1), label_src)
                pred_src = self.cls_fc_son5(data_src)
                cls_loss += 0.5*F.nll_loss(F.log_softmax(pred_src, dim=1), label_src)
            ...
            if mark == 5:
                ...
                data_src = self.sonnet5(data_src)
            ...
            fea4 = self.sonnet4(data)
            pred4 = self.cls_fc_son4(fea4)
            
            fea5 = self.sonnet5(data)
            pred5 = self.cls_fc_son5(fea5)
            
            return pred1, pred2, pred3, pred4, pred5

class MDAdomnet(nn.Module):

    def __init__(self, domain_num=5):
        ...
        self.domain.add_module('fc3', nn.Linear(1024, domain_num))

        #self.softmax = nn.Softmax(dim=1)
        self.avgpool = nn.AvgPool2d(7, stride=1)
        #self.classes = num_classes

    def forward(self, data_src1, data_src2 = 0, data_src3 = 0, data_src4 = 0, data_src5 = 0):
        ...
        if self.training == True:
            ...
            data_src4 = avgp_fea(self, self.sharedNet(data_src4))
            data_src5 = avgp_fea(self, self.sharedNet(data_src5))
            ...
            pred_domain = self.domain(data_src4)
            s_label = 3*torch.ones(data_src1.shape[0]).long().cuda()
            cls_loss4 = F.nll_loss(F.log_softmax(pred_domain, dim=1), s_label)
            
            pred_domain = self.domain(data_src5)
            s_label = 4*torch.ones(data_src1.shape[0]).long().cuda()
            cls_loss5 = F.nll_loss(F.log_softmax(pred_domain, dim=1), s_label)
            
            cls_loss = cls_loss1 + cls_loss2 + cls_loss3 + cls_loss4 + cls_loss5
...
class TSSnet(nn.Module):
    ...
    def forward(self, data_src, data_srct=0, data_tst=0, 
        ...
        if self.training == True:
            ...
            pred_tgt4 = self.cls_fc_son4(data_tgt4)
            
            data_tgt5 = self.sonnet5(data_tgt)
            pred_tgt5 = self.cls_fc_son5(data_tgt5)
            
            data_srct1 = self.sonnet1(data_srct)           
            data_srct2 = self.sonnet2(data_srct)           
            data_srct3 = self.sonnet3(data_srct)          
            data_srct4 = self.sonnet4(data_srct)           
            data_srct5 = self.sonnet5(data_srct)                
            ...
            if mark == 1:                
                ...
                l1_loss += torch.mean( torch.abs(torch.nn.functional.softmax(pred_tgt1, dim=1)
                                                - torch.nn.functional.softmax(pred_tgt5, dim=1)) )
                ...
                cls_loss += F.nll_loss(F.log_softmax(pred_srct, dim=1), label_srct)
                pred_srct = self.cls_fc_son4(data_srct4)                
                cls_loss += F.nll_loss(F.log_softmax(pred_srct, dim=1), label_srct)
                pred_srct = self.cls_fc_son5(data_srct5)                
                cls_loss += F.nll_loss(F.log_softmax(pred_srct, dim=1), label_srct)
                #'''
                ...
                return domain_loss, class_loss, cls_loss, l1_loss/4
            ...
            if mark == 4:
                ...
                data_src = self.sonnet4(data_src)
                data_tst4 = self.sonnet4(data_tst)
                ...
                tar_loss += mmd.mmd(data_srct4, data_tst4)
                st_loss += mmd.mmd(data_src, data_tgt4)
                
                intra_loss0, inters_loss0, intert_loss0 = cal_cls_loss(self, data_src, data_tgt4, pred_src, pred_tgt4)                    
                intra_loss += intra_loss0
                inters_loss += inters_loss0
                intert_loss += intert_loss0
            ...
            if mark == 5:
                ...
                data_src = self.sonnet5(data_src)
                data_tst5 = self.sonnet5(data_tst)
                                                              
                l1_loss = torch.mean( torch.abs(torch.nn.functional.softmax(pred_tgt5, dim=1)
                                                - torch.nn.functional.softmax(pred_tgt1, dim=1)) )
                l1_loss += torch.mean( torch.abs(torch.nn.functional.softmax(pred_tgt5, dim=1)
                                                - torch.nn.functional.softmax(pred_tgt2, dim=1)) )
                l1_loss += torch.mean( torch.abs(torch.nn.functional.softmax(pred_tgt5, dim=1)
                ...
                tar_loss += mmd.mmd(data_srct5, data_tst5)
                st_loss += mmd.mmd(data_src, data_tgt5)
                
                intra_loss0, inters_loss0, intert_loss0 = cal_cls_loss(self, data_src, data_tgt5, pred_src, pred_tgt5)                    
                intra_loss += intra_loss0
                inters_loss += inters_loss0
                intert_loss += intert_loss0
