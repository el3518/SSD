...
list_samtrain = ['_train','tra']
list_samtest = ['_test','tst']
list_tra = list_samtrain[1]
list_tst = list_samtest[0]
...
rate = [0.2,0.2,0.1,0.05,0.05,0.1,0.6]


if sel_flag == 0:
    target_loader = data_loader.load_image_TSS_val(args.root_path, args.test_dir, classlist, list_tra, rate[6], args.batch_size, kwargs)
    source1_loader = data_loader.load_image_TSS_val(args.root_path, args.source1_dir, classlist, list_tst, rate[task[0]], args.batch_size, kwargs)    
    source2_loader = data_loader.load_image_TSS_val(args.root_path, args.source2_dir, classlist, list_tst, rate[task[1]], args.batch_size, kwargs)    
    source3_loader = data_loader.load_image_TSS_val(args.root_path, args.source3_dir, classlist, list_tst, rate[task[2]], args.batch_size, kwargs)    
    source4_loader = data_loader.load_image_TSS_val(args.root_path, args.source4_dir, classlist, list_tst, rate[task[3]], args.batch_size, kwargs)    
    source5_loader = data_loader.load_image_TSS_val(args.root_path, args.source5_dir, classlist, list_tst, rate[task[4]], args.batch_size, kwargs)    
    
    test_num = len(target_loader['val'].dataset)

if sel_flag == 1:
    source_select_loader = data_loader.load_image_TSS_select(args.root_path, source_dir, classlist, '_train', args.batch_size, kwargs)
    source_num = len(source_select_loader.dataset)


K = 1 # training times
train_tags = ['base_sf1', 'select']
...
def test_source(model):#sort(w)
    ...
    for data, target in source1_loader['tra']:
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data), Variable(target)
        bin_label = F.one_hot(target, args.num_class) 
        ...
        '''    
        #correct += pred.eq(bin_label.data.view_as(pred)).cpu().sum()
    print('\nsource {}/{}'.format(correct, len(source1_loader['tra'].dataset)))
               
    correct = 0    
    
    for data, target in source2_loader['tra']:
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data), Variable(target)
        bin_label = F.one_hot(target, args.num_class) 
    ...
    #print('\nloss {:.6f}'.format(t_loss))  
    #print('\nacc {}'.format(correct)#\nsource {}/{}, source_num, correct
    print('\nsource {}/{}'.format(correct, len(source2_loader['tra'].dataset)))
    
    correct = 0    
    
    for data, target in source3_loader['tra']:
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data), Variable(target)
        bin_label = F.one_hot(target, args.num_class) 
        ...
        #for i in range(args.num_class):
            #pred[i] = torch.nn.functional.softmax(pred[i], dim=1)
        correct += np.equal(np.argmax(variable_to_numpy(pred), 1), np.argmax(variable_to_numpy(bin_label), 1)).sum()
    print('\nsource {}/{}'.format(correct, len(source3_loader['tra'].dataset)))    
    
    correct = 0    
    
    for data, target in source4_loader['tra']:
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data), Variable(target)
        bin_label = F.one_hot(target, args.num_class) 
        ...
        #for i in range(args.num_class):
            #pred[i] = torch.nn.functional.softmax(pred[i], dim=1)
        correct += np.equal(np.argmax(variable_to_numpy(pred), 1), np.argmax(variable_to_numpy(bin_label), 1)).sum()
    print('\nsource {}/{}'.format(correct, len(source4_loader['tra'].dataset)))
       
    correct = 0    
    
    for data, target in source5_loader['tra']:
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data), Variable(target)
        bin_label = F.one_hot(target, args.num_class) 
    ...
    #print('\nloss {:.6f}'.format(t_loss))  
    #print('\nacc {}'.format(correct)#\nsource {}/{}, source_num, correct
    print('\nsource {}/{}'.format(correct, len(source5_loader['tra'].dataset)))
...
if __name__ == '__main__':
    ...
    if sel_flag == 1:
        ...
        org_file = datapath + source_dir + '_train.txt'
