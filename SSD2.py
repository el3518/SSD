...
import backbone_alex as backbone
    ...
    def forward(self, data_src, label_src = 0, mark = 1):
        ...
        if self.training == True:
            ...
            if mark == 1:
                ...
                #cls_loss = CrossEntropyLabelSmooth(num_classes=self.classes, epsilon=0.1)(pred_src, label_src)
                #cls_loss += 0.5*CrossEntropyLabelSmooth(num_classes=self.classes, epsilon=0.1)(pred_srca, label_src)            
...
class MDAdomnet(nn.Module):
    ...
    def __init__(self, num_classes=31):
        ...
        self.domain.add_module('fc3', nn.Linear(1024, 2))
        
        self.avgpool = nn.AvgPool2d(7, stride=1)

    def forward(self, data_src1, data_src2 = 0):
...
class MDAnetex(nn.Module):

    def __init__(self, num_classes=31):
        super(MDAnetex, self).__init__()
        self.sharedNet = resnet50(True)
    ...
    def forward(self, data_src, mark = 1):

        data_src = self.sharedNet(data_src)
        if mark==1:           
            data_src = self.sonnetc1(data_src)
            return data_src
            
        if mark==2:           
            data_src = self.sonnetc2(data_src)
            return data_src
        
        if mark==9:
            fea1 = self.sonnet1(data_src)
            fea2 = self.sonnet2(data_src)
...
class TSSnet(nn.Module):
    ...
    def forward(self, data_src, data_srct=0, data_tst=0, 
                data_tgt = 0, label_src = 0, label_srct = 0, mark = 1, test = False):
        ...
        if self.training == True:
            ...
            if mark == 1:                               
                ...
                return domain_loss, class_loss, cls_loss, l1_loss#
            ...
            if test:
                ...
                return fea1, fea2, (pred1+pred2)/2
