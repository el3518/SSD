...
tra_flag = 1
rate = [0.6,0.3,0.3,0.3] #control sample balance
#rate = [0.2,0.15,0.9,0.9]
if tra_flag == 1:
...
def train(traepo,model):
    ...
    s_num = [len(source1_loader['tra'].dataset), len(source2_loader['tra'].dataset), len(source3_loader['tra'].dataset)]
    ...
    for i in range(1, args.iter + 1):#i=100
        ...
        cls_loss = model(source_data1, source_data2, source_data3)
        cls_loss.backward()
        optimizer.step()
        ...
        if i % (args.log_interval * 10) == 0:#
            ...
            for j in range(len(w)):#i=0
                if w1[j]>w2[j] and w1[j]>w3[j]:
                    count[0] = count[0]+1
                if w2[j]>w1[j] and w2[j]>w3[j]:
                    count[1] = count[1]+1
                if w3[j]>w2[j] and w3[j]>w1[j]:
                    count[2] = count[2]+1
            print(count)
...
def test(traepo, model):#sort(w)
    ...
    print('\nloss1 {:.6f}, loss2 {:.6f}, loss3 {:.6f}'.format(t_loss1, t_loss2, t_loss3))  
    print('\nsource1 {}/{}, source2 {}/{}, source3 {}/{}'.format(correct1, source1_num, correct2, source2_num, correct3, source3_num))
...
def weight(model):#sort(w)
    ...
    for data, target in weight_loader:
        ...
        for j in range(pred.shape[0]):
            weight.append([w[j,0], w[j,1], w[j,2]])
