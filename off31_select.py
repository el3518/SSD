...
if sel_flag == 0:
    target_loader = data_loader.load_image_TSS_val(args.root_path, args.test_dir, classlist, sel_samtra, rate[task[2]], args.batch_size, kwargs)
...
K = 1 # training times
train_tags = ['base_sf_alex', 'select_alex']
...
if __name__ == '__main__':
    ...
    if sel_flag == 1:
        traepo = 0
        model = models.Discriminator_alex(num_classes)
        if args.cuda:
            model.cuda()
        ...
        #np.savetxt('./MDA1/{}_src_sel_{}_{}.csv'.format(dataname, args.test_dir, source_dir), np.array(out_pred), fmt='%.6f', delimiter=',')      
